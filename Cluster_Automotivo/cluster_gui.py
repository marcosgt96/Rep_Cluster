...
<div class="cluster" id="cluster">
  ...
  <!-- ─── LEFT PANEL: RPM ──────────────────────────────── -->
  <div class="panel" style="grid-column:1; grid-row:2; animation-delay:0.1s">
    <span class="panel-title">TACHOMETER</span>
  ...
  <div class="panel panel-center" style="grid-row:2;">
    <span class="panel-title">VELOCIDADE</span>
    <canvas id="speedGauge" width="280" height="280"></canvas>
    <div style="position:absolute; bottom: 22%; display:flex; flex-direction:column; align-items:center;">
  ...
  <!-- ─── RIGHT PANEL: GAUGES ──────────────────────────── -->
  <div class="panel" style="grid-column:3; grid-row:2; gap:10px; animation-delay:0.15s">
    ...
    <div class="bar-gauge" style="width:90%;">
      <div class="bar-header">
        <span class="bar-name">Temp. Oleo Motor</span>
      ...
      <div class="bar-ticks">
        ...
        <span class="bar-tick">110</span>
        <span class="bar-tick">140</span>
        <span class="bar-tick">170</span>
      </div>
    </div>
    ...
    <div class="bar-gauge" style="width:90%;">
      <div class="bar-header">
        <span class="bar-name">Temp. Oleo Trans.</span>
        <span><span class="bar-reading" id="transTemp-val">78</span><span class="bar-unit">C</span></span>
      </div>
      <div class="bar-track">
        <div class="bar-fill" id="transTemp-bar" style="width:45%; background: linear-gradient(90deg, var(--blue), #3ab5ff);"></div>
      </div>
      <div class="bar-ticks">
        <span class="bar-tick">30</span>
        <span class="bar-tick">60</span>
        <span class="bar-tick">90</span>
        <span class="bar-tick">120</span>
        <span class="bar-tick">150</span>
      </div>
    </div>
    ...
    <div style="display:flex; align-items:center; justify-content:space-between; width:90%;">
      <div class="bar-gauge" style="flex:1;">
        <div class="bar-header">
          <span class="bar-name">Turbo / Boost</span>
          <span><span class="bar-reading" id="boost-val">0.8</span><span class="bar-unit">bar</span></span>
        </div>
        <div class="bar-track">
          <div class="bar-fill" id="boost-bar" style="width:53%; background: linear-gradient(90deg, var(--accent2), var(--warn));"></div>
        </div>
        <div class="bar-ticks">
          <span class="bar-tick">-1</span>
          <span class="bar-tick">0</span>
          <span class="bar-tick">0.5</span>
          <span class="bar-tick">1.0</span>
          <span class="bar-tick">1.5</span>
        </div>
      </div>
    </div>
    ...
    <div style="display:flex; align-items:center; gap:16px; width:90%;">
      <div style="display:flex; flex-direction:column; align-items:flex-start; flex:1;">
        <span class="bar-name">Tensao Bateria</span>
...
function drawArcGauge(ctx, cx, cy, r, startAngle, endAngle, value, minVal, maxVal, opts = {}) {
  ...
  for (let i = 0; i <= tickCount; i++) {
    ...
    const inner = r - arcWidth - (isMajor ? 18 : 10);
    const outer = r - arcWidth - 2;
    ctx.save();
    ctx.beginPath();
...
function renderRPM(val) {
  ...
  drawArcGauge(rpmCtx, cx, cy, r, START_ANGLE, END_ANGLE, val, 0, 8000, {
    arcWidth: 10,
    fillColor: val > 6500 ? '#e8200a' : val > 5500 ? '#ffcc00' : '#e8630a',
    needleColor: '#ffffff',
    ticks: 8,
    zones: [
      { from: 0,    to: 3000, color: '#0ae86b' },
      { from: 3000, to: 6000, color: '#e8630a' },
      { from: 6000, to: 8000, color: '#e8200a' },
    ]
  });
  ...
  rpmCtx.fillStyle = 'rgba(200,208,224,0.3)';
...
function renderSpeed(val) {
  ...
  drawArcGauge(speedCtx, cx, cy, r, START_ANGLE, END_ANGLE, val, 0, 280, {
    arcWidth: 13,
    fillColor: val > 200 ? '#e8200a' : val > 120 ? '#ffcc00' : '#e8630a',
    needleColor: '#ffffff',
    ticks: 14,
    zones: [
      { from: 0,   to: 100, color: '#0ae86b' },
      { from: 100, to: 200, color: '#e8630a' },
      { from: 200, to: 280, color: '#e8200a' },
    ]
  });

  // Speed labels
  speedCtx.save();
  speedCtx.font = '600 11px Orbitron, monospace';
  speedCtx.fillStyle = 'rgba(200,208,224,0.4)';
  speedCtx.textAlign = 'center';
  [0, 40, 80, 120, 160, 200, 240, 280].forEach((s, i) => {
    const frac = s / 280;
    const a = START_ANGLE + frac * (END_ANGLE - START_ANGLE);
    const lbl_r = r - 13 - 22;
...
function updateRPMLeds(rpm) {
  ...
  for (let i = 0; i < LED_COUNT; i++) {
    ...
    if (i < lit) {
      if (rpm > 6500) el.classList.add('on-red');
      else if (rpm > 5500) el.classList.add('on-yellow');
...
function updateDisplay(data) {
  ...
  // Oil temp
  document.getElementById('oiltemp-val').textContent = Math.round(data.oil_temp);
  const oilPerc = ((data.oil_temp - 40) / (170 - 40)) * 100;
  document.getElementById('oiltemp-bar').style.width = Math.max(0, Math.min(100, oilPerc)) + '%';

  // Trans temp
  document.getElementById('transTemp-val').textContent = Math.round(data.trans_temp);
  const transPerc = ((data.trans_temp - 30) / (150 - 30)) * 100;
  document.getElementById('transTemp-bar').style.width = Math.max(0, Math.min(100, transPerc)) + '%';

  // Boost
  document.getElementById('boost-val').textContent = data.boost.toFixed(1);
  const boostPerc = ((data.boost + 1) / (1.5 + 1)) * 100;
  document.getElementById('boost-bar').style.width = Math.max(0, Math.min(100, boostPerc)) + '%';
...
function startSimulation() {
  simulationInterval = setInterval(() => {
    const data = {
      ...
      oil_temp: 80 + Math.random() * 40,
      trans_temp: 60 + Math.random() * 40,
