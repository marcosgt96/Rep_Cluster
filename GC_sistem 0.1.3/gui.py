...
class SellerTab(ttk.Frame):
    def __init__(self, parent):
        ...
        ttk.Button(frm, text="✏️ Editar", command=self.edit_seller).pack(side='left', padx=5)
        ttk.Button(frm, text="🗑️ Excluir", command=self.remove_seller).pack(side='left', padx=5)
    ...
    def edit_seller(self):
        sel = self.tree.focus()
        if not sel:
            messagebox.showwarning("Aviso", "Selecione um vendedor na lista")
            return
        seller_id, old_name = self.tree.item(sel)['values']
        new_name = simpledialog.askstring("Editar", "Novo nome para o vendedor:",
                                         initialvalue=old_name, parent=self)
        if not new_name or new_name == old_name:
            return
        ok = update_seller_name(seller_id, new_name)
        if ok:
            messagebox.showinfo("✅ Sucesso", "Nome atualizado!")
            self.load_sellers()
        else:
            messagebox.showerror("❌ Erro", "Falha ao atualizar nome (provavelmente já existe)")
    ...
    def remove_seller(self):
        ...
        if not sel:
            messagebox.showwarning("Aviso", "Selecione um vendedor na lista")
            return
        seller_id, name = self.tree.item(sel)['values']
        if not messagebox.askyesno("Confirmação", f"Excluir vendedor '{name}'?"):
            return
        ok = delete_seller(seller_id)
        if ok:
            messagebox.showinfo("✅ Sucesso", "Vendedor excluído!")
            self.load_sellers()
        else:
            messagebox.showerror("❌ Erro", "Não é possível excluir vendedor com vendas")
