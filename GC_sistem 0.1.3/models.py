...
def delete_seller(seller_id):
    """Remove a seller if there are no sales linked to them.
    Returns True on success, False if the seller has sales or does not exist.
    """
    conn = get_connection()
    cur = conn.cursor()
    # check for existing sales
    cur.execute("SELECT COUNT(*) FROM sales WHERE seller_id = ?", (seller_id,))
...
def update_seller_name(seller_id, new_name):
    """Change the name of a seller. Returns True on success, False if the name
    is already in use or seller does not exist.
    """
    conn = get_connection()
    cur = conn.cursor()
    try:
        cur.execute("UPDATE sellers SET name = ? WHERE id = ?", (new_name, seller_id))
        if cur.rowcount == 0:
            conn.close()
            return False
        conn.commit()
        ...
        # likely UNIQUE constraint failure or other integrity issue
        conn.close()
        return False
